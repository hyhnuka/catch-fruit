...
connection.on("GameStartForPlayer", (playerName) => {
    ...
    // Sembunyikan layar lobby dan layar game over
    if (startScreen) startScreen.classList.add("hidden");
    if (gameOverScreen) gameOverScreen.classList.add("hidden");
    if (gameScreen) gameScreen.classList.remove("hidden");

    // Munculkan popup transisi
    overlay.classList.remove("hidden");
    title.textContent = `Giliran ${playerName}!`;
    sub.textContent = "Bersiap mengendalikan keranjang dari HP...";

    // Jeda 2.5 detik agar pemain sempat bersiap
    setTimeout(() => {
...
class Game {
    ...
    // Dipanggil saat permainan selesai
    gameOver(message = "Game Over!") {
        ...
        // Tahan layar Game Over selama 3.5 detik agar pemain melihat skornya
        // Setelah itu, panggil antrean pemain berikutnya di backend
        setTimeout(() => {
    ...
    start() {
        ...
        // Jalankan countdown timer permainan
